...
def fetch_uniprot_species_data(extended: bool = False) -> List[Dict]:
    ...
    # Select organism set based on extended flag
    organisms = COMMON_ORGANISMS.copy()
    if extended:
        organisms.update(EXTENDED_ORGANISMS)
        logger.info("Using extended organism list")
    else:
...
def main():
    ...
    parser.add_argument(
        '--extended',
        action='store_true',
        help='Include extended list of organisms (default: common model organisms only)'
    )
    parser.add_argument(
        '--output',
        type=str,
